...
class Word2VecPreTrainBaseExperiment(Experiment):
    ...
    def __init__(self, dataset, experiment_parameters):
        ...
        # Embedding
        self.w2v_model = Word2VecEventEmbedder()
        self.w2v_model.load_model(self.experiment_parameters["pre_train_embedding"])
    ...
    def get_embedding_matrix(self, vocabulary):

        oov_vector = np.zeros(self.w2v_model.embedding_size)

        embedding_weight = self.w2v_model.get_vectors_for_vocabulary(vocabulary)

        embedding_weight = np.vstack([oov_vector, embedding_weight])

        embedding_weight = np.array(embedding_weight, dtype='float32')
