...
class ELMoBiLSTMExperiment_2_dense(ELMoPreTrainBaseExperiment):
    ...
    def build_model_classifier(self, run_number=0):
        ...
        # build the model
        lambda_1 = tf.Variable(1.)
        lambda_2 = tf.Variable(1.)
        lambda_3 = tf.Variable(1.)
        ...
        elmo_output = elmo_embedding_layer (input_model)
        
        tokens_emb = lambda_1*elmo_output[0] + lambda_2*elmo_output[1] + lambda_3*elmo_output[2]
        
        #lambda_1 = Dense(1, activation='relu')
        #l_1 = Multiply()([lambda_1, elmo_output[0]])
        
        #lambda_2 = Dense(1, activation='relu')
        #l_2 = Multiply()([lambda_2, elmo_output[1]])
        
        #lambda_3 = Dense(1, activation='relu')
        #l_3 = Multiply()([lambda_3, elmo_output[2]])

        #tokens_emb = Add()([l_1, l_2, l_3])

        lstm_1 = Bidirectional(LSTM(output_dim))(tokens_emb)
        
        output_layer = Dense(output_dim*4, activation='relu')(lstm_1)

        output_layer = Dense(nb_classes, activation='softmax')(output_layer)

        self.classifier_model = Model(inputs=input_model, outputs=output_layer, name="ELMo_BiLSTM_Classifier_V2_dense")
